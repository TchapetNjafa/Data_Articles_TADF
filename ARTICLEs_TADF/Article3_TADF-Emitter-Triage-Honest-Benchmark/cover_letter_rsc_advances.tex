...
\num{0.049}~eV, about half the model error---against which a \num{609}-core-hour
TD-DFT reference and a frozen pretrained molecular-transformer embedding both fail
to beat the structure-only model. Applying a noise-ceiling calculation
(Crusius~\textit{et al.}) to that precision gives $R^2_\text{max} \approx
0.71$--\num{0.91} depending on which noise floor is used; the observed $R^2 =
0.26$ reaches only \num{28}--\num{36}\% of that ceiling, so label imprecision
bounds what is achievable but does not by itself explain the shortfall.
(iv)~A triage filter that enriches low-gap candidates by
\num{1.2}--\num{1.4}$\times$ over random on held-out labelled data, though a
bootstrap confidence interval shows this is resolved from chance only at the
broader top-\num{50}/top-\qty{20}{\percent} cutoffs, not at the top-\num{10}/top-\num{20}
scale a laboratory would actually shortlist from---useful for coarse
...
though a bootstrap confidence interval shows this is resolved from chance only at
the top-\num{50}/top-\qty{20}{\percent} cutoffs and not at the top-\num{10}/top-\num{20} scale a
laboratory would actually shortlist from),
...
We have also quantified, directly, how much of the residual gap even a perfect
model could close. Applying the noise-ceiling framework of
Crusius~\textit{et al.} to the label's own measurement precision gives
$R^2_\text{max} \approx 0.71$--\num{0.91} (depending on whether the single-report
or median-precision floor is used), so the observed $R^2 = 0.26$ reaches only
\num{28}--\num{36}\% of what label imprecision alone would permit. Label noise
therefore sets a real, quantified ceiling on $R^2$, but it does not by itself
explain why the model sits well below even that ceiling; we now state both
halves of that argument explicitly (\S\,Ceilings).
...
plainly---a noise-ceiling calculation (Crusius~\textit{et al.}) puts the maximum
achievable $R^2$ at \num{0.71}--\num{0.91} given the label's own measurement
precision, and the observed $R^2 = 0.26$ reaches only \num{28}--\num{36}\% of that.
For triage: yes, within a stated bound---\num{1.2}--\num{1.4}$\times$
enrichment of low-gap emitters over random selection on held-out labelled data,
though a bootstrap CI resolves this from chance only at the
top-\num{50}/top-\qty{20}{\percent} cutoffs, not at the top-\num{10}/top-\num{20} scale,
