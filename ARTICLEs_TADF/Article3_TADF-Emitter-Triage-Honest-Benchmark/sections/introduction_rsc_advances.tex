...
artefact. The excited-state step buys no accuracy on the $S_1$--$T_1$ gap---the two
agree to within \qty{0.017}{\electronvolt} on paired folds---though it does retain rank
correlation across source papers where the fingerprints do not; SHAP nonetheless
...
experimental reports for the same compound scatter by \qty{0.072}{\electronvolt} RMS
(the worst molecule reaching a standard deviation of \qty{0.56}{\electronvolt}), so the
median gap the model is trained on is itself known only to \qty{0.049}{\electronvolt}.
A separate limit applies to computed references, which are functional-dependent (B3LYP vs
CAM-B3LYP differ by up to \qty{0.58}{\electronvolt}); that bounds what a computed gap
can mean, not what a model fitted to measured labels can achieve. The ceiling is not an excuse: no higher-capacity learner
...
(\numrange{1.2}{1.4}$\times$ over random selection) on held-out labelled data;
on the filtered library the conformal prediction intervals are too wide to
support molecule-level selection, so no ranked shortlist is deposited.
$\Delta E_\text{ST}$ is necessary but not sufficient for TADF efficiency;
