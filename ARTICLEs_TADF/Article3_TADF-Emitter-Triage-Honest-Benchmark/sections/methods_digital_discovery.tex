...
\qty{0.072}{\electronvolt} (RMS) from the median and this scatter bounds achievable
accuracy (\cref{subsec:ceilings}). The final benchmark comprises \num{231} donor--acceptor
...
(\texttt{xtb} 6.7.1) followed by sTDA~\cite{Grimme2017} for the lowest singlet
...
fitted forest. All scripts (\texttt{ML\_reproducibility/code/finalize\_model.py},
\texttt{ML\_reproducibility/code/rebuild\_structural\_model.py}) regenerate every reported number and
...
out-of-fold predictions (\texttt{ML\_reproducibility/code/r2\_ci\_power\_analysis.py}).
...
(DOI: 10.5281/zenodo.17436070) under a CC-BY-4.0 licence, so that every number and
