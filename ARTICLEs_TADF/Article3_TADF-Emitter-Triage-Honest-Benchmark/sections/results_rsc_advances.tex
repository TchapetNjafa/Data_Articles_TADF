...
severe the leak looks depends on the learner: a random forest, being axis-aligned,
approximates a difference of two continuous inputs through its splits rather than
expressing it, and reaches only $R^2 = 0.59$ on the same leaked features (a joint
effect of that inductive bias, tree depth and the \num{231}-molecule sample)---which
is how a tree-based study can carry this defect without its cross-validated score
appearing anomalous. We therefore discard the
computed gap as a target and benchmark on the only non-trivial quantity: the
\emph{experimentally measured} $\Delta E_\text{ST}$. This risk is not particular to
TADF: any regression target that is an algebraic combination of features already in
the descriptor set is vulnerable to the same leak, and how visible it is under
cross-validation depends on the learner, not only on the target.
...
while preserving the rank correlation (Spearman $\rho$ spanning \numrange{0.31}{0.40} across cutoffs), showing that the ranking capability does not degrade under aggressive clustering
...
Equivalently, the spatial NTO features carry no information beyond what the energy
difference already provides: the MAE of the \emph{Energies~+~NTO} set (\qty{0.097}{\electronvolt})
matches the spatial-only NTO set (\qty{0.096}{\electronvolt}) to \qty{0.001}{\electronvolt}.
...
hole--electron overlap $S_\text{he}$ (which governs the magnitude of the exchange interaction), the NTO overlap ratio (tracking transition purity), oscillator strength
$f_{S_1}$ (a proxy for transition dipole coupling), and the charge-transfer distance $\Delta r$ (which dictates spatial separation) as the dominant
predictors---physical quantities that jointly constrain the exchange integral governing
...
\subsection{Accuracy is equivalent; ranking under laboratory transfer is not}\label{subsec:nto_null}

Do quantum-mechanical features beat pure structural descriptors when the target is an
experimental property? For accuracy, no, and we can put a bound on the equivalence.
...
\qtyrange{0.01}{0.48}{\electronvolt} (\cref{SI-tab:solvent}). This scatter reflects uncontrolled measurement conditions---such as solvent polarity, concentration, and host matrix effects---that are known to heavily influence photophysical measurements~\cite{Dang2025}. That $\Delta E_\text{ST}$
is reported inconsistently has been shown for individual emitters~\cite{Shuaib2026};
quantified across \num{231} molecules and \num{173} source papers it is, to our
knowledge, the first inter-laboratory variability budget for the TADF
singlet--triplet gap, and it is the term that caps every predictor tested here.
...
Applying the noise-ceiling framework of \citeauthor{Crusius2025}~\cite{Crusius2025}
($R^2_\text{max} = 1 - \sigma^2_\text{noise}/\sigma^2_y$) quantifies this:
the \qty{0.049}{\electronvolt} median precision gives $R^2_\text{max} \approx 0.91$
(\qty{95}{\percent} CI \numrange{0.85}{0.94}), and the single-report scatter
(\qty{0.091}{\electronvolt}) gives $R^2_\text{max} \approx 0.71$
(CI \numrange{0.48}{0.81})---both well above the observed $R^2 = 0.26$.
The model captures only 28--36\% of the noise-floor-limited ceiling depending on which
floor applies, meaning label noise alone is not the dominant constraint.
Progress requires both a better dataset \emph{and} improvements in features or
corpus scale; the data argument is justified because the label precision today
(\qty{0.049}{\electronvolt}) is the term a curated benchmark would directly attack.
...
histogram-boosting configuration). A frozen pretrained SMILES transformer (ChemBERTa,
\num{77}M parameters), the representation a current screening pipeline would reach for
first, is no exception: its mean-pooled embeddings feed a random-forest or ridge head
to MAE \qtyrange{0.109}{0.113}{\electronvolt} on the same scaffold folds---it does not
beat the \qty{0.107}{\electronvolt} mean-value baseline, and is worse than Morgan by a
paired \qty{0.022}{\electronvolt} (\qty{95}{\percent} CI \numrange{0.008}{0.034},
excluding zero; \texttt{code/a013\_pretrained\_encoder\_baseline.py}). Fine-tuning such an
encoder on \num{231} labels would overfit and is not a meaningful test at this scale.
Added model capacity therefore does not help, and no learned representation we tried
improves on the hand-built descriptors, though we would not claim the forest is
uniquely suited; and the negative lower $R^2$ bound is not a
...
confidence interval on this cross-validation enrichment, given below, shows the smaller cutoffs are
...
first pass that modestly concentrates good emitters, not a selector. Part of that
weakness is the threshold itself: \qty{0.10}{\electronvolt} is only
\qty{0.03}{\electronvolt} above the \qty{0.072}{\electronvolt} RMS inter-report
scatter (\cref{subsec:ceilings}), so a molecule's true class membership near that
line is itself uncertain at the level of label noise---tightening the triage
threshold runs directly into the same ceiling that bounds the regression task.
...
class by \numrange{1.2}{1.4}$\times$ over random selection, but a
\qty{95}{\percent} bootstrap confidence interval (\num{5000} resamples over
molecules, the ranking re-derived from the fixed cross-validation out-of-fold predictions within
each resample) shows this holds only at the less aggressive cutoffs. Against the
\qty{49}{\percent} base rate ($\Delta E_\text{ST} \le \qty{0.10}{\electronvolt}$),
top-ranked precision reaches \qty{60}{\percent} at both the top \num{10} and top
\num{20} candidates (enrichment \num{1.22}$\times$, \qty{95}{\percent} CI
\numrange{0.63}{1.89} and \numrange{0.80}{1.70}: both intervals include no
enrichment), and \qty{68}{\percent} at the top \num{50} (\num{1.38}$\times$, CI
\numrange{1.10}{1.60}) or \qty{67.4}{\percent} at the top \qty{20}{\percent} of the
benchmark (\num{1.37}$\times$, CI \numrange{1.08}{1.61}: both intervals exclude no
enrichment). The enrichment is therefore only statistically established at cutoffs
of roughly \num{50} molecules or a fifth of the benchmark; at the ten-to-twenty
candidate scale a laboratory would actually shortlist, it cannot be distinguished
from chance at this sample size. The full enrichment curve
...
On the model side, the comparison covers tabular learners and one frozen pretrained
molecular-transformer embedding (\cref{subsec:ceilings}); an end-to-end message-passing
network or an encoder fine-tuned on a larger labelled set was not attempted, and either
would need a benchmark larger than \num{231} molecules to be informative.
