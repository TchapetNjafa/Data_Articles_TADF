...
accuracy against such a reference is not physically meaningful. Combined as independent
contributions, the inter-report scatter (\cref{SI-tab:solvent}) and the residual
vertical-descriptor offset (\cref{SI-tab:adiabatic}) place the expected error floor near
\qty{0.13}{\electronvolt} by root-sum-square, matching the observed MAE of
\qty{0.096}{\electronvolt}. These two ceilings,
...
either, so the floor is intrinsic to the signal available, not a fixable modelling choice.
...
at the upper \num{1.4}$\times$. For a laboratory processing \num{1100} compounds, this is
...
compounds with a reported inverted gap (deposited, \texttt{data/ist\_candidates.csv});
